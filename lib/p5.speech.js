...
// =============================================================================
  ...
  p5.Speech.prototype.setVoice = function(_v) {
    ...
    if(typeof(_v)=='string') this.voice = this.voices.filter(function(v) { return v.name == _v; })[0];
    else if(typeof(_v)=='number') this.voice = this.voices[Math.min(Math.max(_v,0),this.voices.length-1)];
  };

  // volume of voice. API range 0.0-1.0.
  p5.Speech.prototype.setVolume = function(_v) {
    this.volume = Math.min(Math.max(_v, 0.0), 1.0);
  };
  ...
  p5.Speech.prototype.setRate = function(_v) {
    this.rate = Math.min(Math.max(_v, 0.1), 2.0);
  };
  ...
  p5.Speech.prototype.setPitch = function(_v) {
    this.pitch = Math.min(Math.max(_v, 0.01), 2.0);
  };
  ...
  p5.Speech.prototype.speak = function(_phrase) {
    if(this.interrupt) this.cancel();
    var utterance = this.newUtterance()
    utterance.text = _phrase;

    this.synth.speak(utterance);
  };

  var speechToUtteranceProperties = {
    interrupt: "interrupt",
    onLoad: "onload",
    onStart: "onstart",
    onPause: "onpause",
  ...
  p5.Speech.prototype.newUtterance = function() {
    var utterance = new SpeechSynthesisUtterance();
    for (var speechProperty in speechToUtteranceProperties) {
      var value = this[speechProperty]
      if (value !== undefined) {
        var utteranceProperty = speechToUtteranceProperties[speechProperty];
        utterance[utteranceProperty] = value;
      }
    }
